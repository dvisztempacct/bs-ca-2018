...
module Make = (Impl: Impl) => {
  ...
  let putExn = (c, x, y, v) =>
    putBool(c, x, y, v) ? () : raise(Lib.Outofrange);
...
module PlainTextCanvas =
  Make({
    ...
    let charOfCell = x => x;
    let rowToString = row =>
      row |> Slice.get |> Array.map(String.make(1)) |> Js.Array.joinWith("");
  });

type ansiColorCell = {
  color: Lib.Ansi.color,
  char,
};

module AnsiColorCanvas =
  Make({
    module A = Lib.Ansi;
    ...
    let rowToString = row =>
      ...
      |> Slice.get
      |> Array.map(x =>
           A.reset ++ A.stringOfColor(x.color) ++ String.make(1, x.char)
         )
      |> Js.Array.joinWith("");
  });
    ...
    for (y in 0 to h - 1) {
      for (x in 0 to w - 1) {
        ...
        (
          ...
          | (false, _) => [|
              {char: a, color: Lib.Ansi.Red},
              {char: b, color: Lib.Ansi.BlackOnRed},
            |]
          }
        )
        |> Array.iteri((i, ch) =>
             AnsiColorCanvas.putExn(c, x * 2 + i, y, ch)
           );
      };
    };
    c;
